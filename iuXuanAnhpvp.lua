-- iuXuanAnhpvp - ULTIMATE PVP EDITION
-- Features: Fixed WalkSpeed, God Mode, Anti-Ban, Water Walk, Skill Aim, Fast Fly
...
ScreenGui.Name = "iuXuanAnhpvp_EN"
...
Main.BorderColor3 = Color3.fromRGB(0, 191, 255)
Main.Active = true
Main.Draggable = true
...
Title.Text = "iuXuanAnhpvp"
Title.BackgroundColor3 = Color3.fromRGB(255, 180, 200)
...
CreateToggle("God Mode", function(v)
    ...
    task.spawn(function()
        ...
        if not godOn and player.Character:FindFirstChild("Humanoid") then
...
CreateToggle("Walk on Water", function(v)
    waterPart.CanCollide = v
    task.spawn(function()
        while v do
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                waterPart.Position = Vector3.new(player.Character.HumanoidRootPart.Position.X, 0, player.Character.HumanoidRootPart.Position.Z)
            end
            task.wait()
        end
    end)
...
local flying = false
local flySpeed = 100
CreateToggle("Fast Fly", function(v)
    flying = v
...
-- 4. Skill Aimlock
local aimOn = false
CreateToggle("Skill Aimlock", function(v) aimOn = v end)
    ...
    for _, v in pairs(game.Players:GetPlayers()) do
        if v ~= player and v.Character then
            if state then
                ...
                h.Name = "iuXA_ESP"; h.FillColor = Color3.fromRGB(255, 180, 200)
            else
                if v.Character:FindFirstChild("iuXA_ESP") then v.Character.iuXA_ESP:Destroy() end
            end
        end
    end
end)
...
CreateSlider("WalkSpeed", 16, 500, 16, function(v) speedVal = v end)
CreateSlider("Fly Speed", 50, 500, 100, function(v) flySpeed = v end)
