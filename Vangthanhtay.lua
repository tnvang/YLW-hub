...
RunService.RenderStepped:Connect(function()
    ...
    if aimEnabled then
        ...
        for _, obj in pairs(workspace:GetDescendants()) do
            if obj:IsA("Model") and obj:FindFirstChild("Humanoid") and obj:FindFirstChild("Head") and obj.Name ~= Player.Name and obj.Humanoid.Health > 0 then
                local pos, onScreen = Camera:WorldToViewportPoint(obj.Head.Position)
                if onScreen then
                    local d = (Vector2.new(pos.X, pos.Y) - mouse).Magnitude
                    if d < dist then dist, target = d, obj.Head end
                end
            end
        end
