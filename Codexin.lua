...
if _G.MobileHubExecutedFinalV4_2 then
    print("[Mobile Hub] Hub V4.2 đã kích hoạt!")
    return
else
...
-- TAB 2: MOVEMENT (DI CHUYỂN & SỬA LỖI FLY)
...
-- TAB 3: VISUALS (HIỆU ỨNG LẤP LÁNH & ĐỔI FOV CAMERA)
...
-- TÍNH NĂNG 1: BẬT TẮT HIỆU ỨNG HẠT VÀ TIA SÁNG CẦU VỒNG
createToggle("Visuals", "HIỆU ỨNG NHÂN VẬT", 4, function(state) toggleVisualEffects(state) end)

-- TÍNH NĂNG 2: THANH KÉO ĐỔI CAMERA FOV
createSlider("Visuals", "Góc Nhìn Camera (FOV)", 30, 120, 70, 5, function(v)
    State.Camera_FOV_Value = v
    Camera.FieldOfView = v
end)

-- THUẬT TOÁN ĐIỀU KHIỂN FLY MOBILE 3D CHUẨN XÁC (ĐÃ SỬA LỖI BAY NGƯỢC)
...
-- THUẬT TOÁN TẠO TIA SÁNG NHIỀU MÀU VÀ HẠT LẤP LÁNH VÂY QUANH
function toggleVisualEffects(enable)
    State.VisualEffects_Enabled = enable
    ...
    if enable then
        ...
        local topAtt = Instance.new("Attachment") topAtt.Position = Vector3.new(0, 2, 0) topAtt.Parent = root
        local bottomAtt = Instance.new("Attachment") bottomAtt.Position = Vector3.new(0, -2, 0) bottomAtt.Parent = root
        ...
        ActiveSparkles.SparkleColor = Color3.fromRGB(255, 255, 255)
        ActiveSparkles.Parent = root
        
        task.spawn(function()
            local hue = 0
            while State.VisualEffects_Enabled and ActiveTrail and ActiveTrail.Parent do
        ...
        if root then
            for _, v in ipairs(root:GetChildren()) do
                if v:IsA("Trail") or v:IsA("Sparkles") or v:IsA("Attachment") then v:Destroy() end
            end
        end
        ActiveTrail = nil
        ActiveSparkles = nil
...
-- Đảm bảo Camera FOV luôn giữ nguyên thông số cài đặt kể cả khi hồi sinh
...
-- LOGIC ESP & HITBOX GIỮ NGUYÊN (NHƯNG ĐƯỢC ĐÓNG GÓI SẠCH SẼ)
...
LocalPlayer.CharacterAdded:Connect(function(char)
    task.wait(1)
    if State.VisualEffects_Enabled then
