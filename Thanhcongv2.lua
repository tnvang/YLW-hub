...
-- 2. Thêm nền Gradient cho MainFrame
...
-- 1. Viền Rainbow cho Khung chính
...
-- CÁC HÀM TẠO THÀNH PHẦN GIAO DIỆN KHUÔN MẪU
local function createToggle(tab, text, order, callback)
    ...
    -- 4. Hiệu ứng Hover cho nút bấm
    local normal = btn.BackgroundColor3
    btn.MouseEnter:Connect(function()
        normal = btn.BackgroundColor3 -- Cập nhật màu hiện tại để tránh lỗi đổi màu khi Toggle ON/OFF
...
-- 3. Animation thu gọn / mở rộng giao diện mượt mà (TweenService)
...
-- MỚI: Hàm đồng bộ và cập nhật danh sách Dropdown tự động
...
-- MỚI: Tự động cập nhật Dropdown ngay khi có ai đó vào hoặc rời server
Players.PlayerAdded:Connect(function() if DropdownFrame.Visible then updateDropdownList() end end)
Players.PlayerRemoving:Connect(function() if DropdownFrame.Visible then updateDropdownList() end end)
...
local function togglePortalSystem(enable) State.Portal_System_Enabled = enable; PortalContainer.Visible = enable; if not enable then currentSelectedId = nil; for id = 1, MAX_PORTALS do deletePortalPair(id) end end end
local PortalMaster_Btn = createToggle("Movement", "HỆ THỐNG PORTAL", 10, function(state) togglePortalSystem(state) end)

-- TAB 3: VISUALS
local toggleFPS, toggleVisualEffects
...
-- THUẬT TOÁN ĐIỀU KHIỂN FLY MOBILE 3D MỚI
...
-- HÀM LOGIC QUẢN LÝ 3 KIỂU HIỆU ỨNG ĐẸP MẮT
...
-- LOGIC AIMBOT TỐI ƯU HÓA (ĐÃ SỬA CẢ QUÁI KHÔNG LAG)
...
function getClosestAimTarget()
    ...
    -- Aim NPC (Đã tối ưu hóa sang GetChildren để loại bỏ hiện tượng Drop FPS)
