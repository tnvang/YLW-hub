...
ScreenGui.Name = "Vagxautr" -- Đổi tên GUI ở đây
...
LoadText.Text = "Khang Gây: Initializing..."
...
task.spawn(function()
    ...
    for percent = 1, 100 do
        if percent % 25 == 0 then
            ...
            LoadText.Text = "Khang Gây: " .. statusTexts[textIdx] .. " (" .. percent .. "%)"
        else
            LoadText.Text = "Khang Gây: Scanning... (" .. percent .. "%)"
        end
        task.wait(0.12)
    end
    LoadFrame:Destroy()
...
local function CreateBox(obj, color)
    if obj:FindFirstChild("KhangGeyBox") then return end
    local box = Instance.new("BoxHandleAdornment")
    box.Name = "KhangGeyBox"
        ...
        if obj:IsA("Model") or obj:IsA("BasePart") then
            ...
            if root and (root.Position - (obj:IsA("Model") and obj:GetPivot().Position or obj.Position)).Magnitude <= 700 then
                ...
                elseif E_Vat and CheckItemESP(name) then
                    ...
                    if obj:FindFirstChild("KhangGeyBox") then obj.KhangGeyBox:Destroy() end
                end
            else
                if obj:FindFirstChild("KhangGeyBox") then obj.KhangGeyBox:Destroy() end
            end
    ...
    if AimBot then
        ...
        for _, v in pairs(Workspace:GetDescendants()) do
            if v:IsA("Model") and v:FindFirstChild("Humanoid") and v ~= Player.Character then
                ...
                if hum.Health > 0 and not game.Players:GetPlayerFromCharacter(v) then
...
ResetBtn.Text = "RESET ALL KHANG GÂY"
...
ResetBtn.MouseButton1Click:Connect(function()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do if obj:FindFirstChild("KhangGeyBox") then obj.KhangGeyBox:Destroy() end end
end)
