...
-- 1. TẠO GUI LOADING THU GỌN (Thương hiệu vangtudong)
local ScreenGui = Instance.new("ScreenGui", Player.PlayerGui)
ScreenGui.Name = "vangtudong_autobond"
...
LoadText.Text = "vangtudong loading."
...
Bond_Btn.MouseButton1Click:Connect(function()
    ...
    Bond_Btn.Text = bondEnabled and "VANGTUDONG: ON" or "VANGTUDONG: OFF"
        ...
        task.spawn(function()
            while bondEnabled do
                ...
                if myChar and myChar:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- Quét diện rộng tìm vật phẩm
                    for _, obj in pairs(Workspace:GetDescendants()) do
                        local nameLower = string.lower(obj.Name)
                        
                        if string.find(nameLower, "trái") or string.find(nameLower, "phiếu") or 
                           string.find(nameLower, "bond") or string.find(nameLower, "money") or 
                           obj:FindFirstChild("ClickDetector") or obj:FindFirstChildWhichIsA("ProximityPrompt") then
                            
                            if not obj:IsDescendantOf(myChar) and not obj:IsDescendantOf(game.Players) then
                                local pos = getPositionOfObject(obj)
                                if pos then
                                    local dist = (root.Position - pos).Magnitude
                                    if dist < shortestDist and dist > 2 then
                                        shortestDist = dist
                                        closestBond = obj
                                    end
                                end
                    ...
                    if closestBond then
                        ...
                        if targetPos then
                            ...
                            -- Thử kích hoạt nhặt bằng mọi cách
                            autoPressE()
                            if closestBond:FindFirstChildWhichIsA("ProximityPrompt") then
                                closestBond:FindFirstChildWhichIsA("ProximityPrompt"):InputHoldBegin()
                                task.wait(0.1)
                                closestBond:FindFirstChildWhichIsA("ProximityPrompt"):InputHoldEnd()
                            end
...
print("vangtudong: Đã sẵn sàng hoạt động!")
