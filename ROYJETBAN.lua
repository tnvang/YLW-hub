-- ROYJETBAN ULTIMATE (FIXED HITBOX, KILL AURA, GOD MODE, FLY)
...
Main.Size = UDim2.new(0,260,0,520) -- Tăng chiều cao để đủ chỗ cho Hitbox
...
QuickKill.Text = "KILL"; QuickKill.TextColor3 = Color3.new(1,1,1)
QuickKill.Font = Enum.Font.SourceSansBold; QuickKill.TextSize = 18
Instance.new("UICorner", QuickKill).CornerRadius = UDim.new(1,0)
...
Container.Size = UDim2.new(1,-10,1,-60); Container.Position = UDim2.new(0,5,0,50)
...
function CreateSlider(name,min,max,default,callback)
    ...
    Fill.Size = UDim2.new((default-min)/(max-min),0,1,0); Fill.BackgroundColor3 = Color3.fromRGB(255,100,150)
...
function CreateToggle(name,default,callback)
    ...
    T.Size = UDim2.new(0.9,0,0,30); T.Text = name.." : "..(default and "ON" or "OFF")
...
RunService.Stepped:Connect(function()
    pcall(function()
        ...
        if _G.Settings.Fly then
            root.Velocity = Vector3.new(0, 2, 0) -- Kháng trọng lực
            if hum.MoveDirection.Magnitude > 0 then
                -- Bay theo đúng hướng camera nhìn
        ...
        pcall(function()
            ...
            for _, v in pairs(Players:GetPlayers()) do
                if v ~= player and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    local tHum = v.Character.Humanoid
                    -- Hitbox Logic
                    tRoot.Size = Vector3.new(_G.Settings.HitboxSize, _G.Settings.HitboxSize, _G.Settings.HitboxSize)
                    tRoot.Transparency = 0.7; tRoot.CanCollide = false
                    -- Kill Aura Logic
                    if _G.Settings.KillAura and tool and tHum.Health > 0 then
                        if (player.Character.HumanoidRootPart.Position - tRoot.Position).Magnitude <= 1000 then
                            tool:Activate()
                            firetouchinterest(tRoot, tool:FindFirstChild("Handle") or tool:FindFirstChildOfClass("Part"), 0)
                            firetouchinterest(tRoot, tool:FindFirstChild("Handle") or tool:FindFirstChildOfClass("Part"), 1)
                        end
                    end
                end
            end
...
local function CreateESP(v)
    ...
    h.FillColor = Color3.fromRGB(255, 0, 127); h.OutlineColor = Color3.new(1,1,1)
    RunService.RenderStepped:Connect(function()
        if _G.Settings.ESP and v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
            h.Adornee = v.Character; h.Enabled = true
...
CreateSlider("Hitbox Size", 2, 100, 10, function(v) _G.Settings.HitboxSize = v end)
