-- [[ GEMINIPRO PREMIUM MENU SCRIPT - FULLY FIXED & OPTIMIZED 2026 ]] --
...
local Options = {
    Aimbot = false,
    AimNPC = false,
    ShowFOV = true, -- Mới: Bật/Tắt hiển thị vòng FOV
    FOVRadius = 120,
    HitboxChance = 100,
    HitboxSize = 2,
    ESP = false,
...
-- Hàm kiểm tra vật cản bằng Raycast chuẩn xác
...
-- Thực hiện khóa mục tiêu mượt mà
RunService.RenderStepped:Connect(function()
...
------------------------------------------------------------------------
-- [ 4. CẬP NHẬT TỐC ĐỘ, NHẢY CAO, BAY & NOCLIP ]
------------------------------------------------------------------------
...
RunService.RenderStepped:Connect(function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        -- Cập nhật WalkSpeed và JumpPower liên tục theo tùy chỉnh
        if hum then
...
HUDFrame.Position = UDim2.new(0.82, 0, 0.02, 0) -- Vị trí góc nhỏ gọn
HUDFrame.Size = UDim2.new(0, 150, 0, 75)       -- Thu nhỏ kích thước tối đa
...
HUDLabel.TextSize = 10 -- Cỡ chữ nhỏ gọn dễ nhìn
...
CreateNewToggle("Hiển thị vòng FOV", "ShowFOV") -- Nút Đóng/Mở vòng FOV vừa thêm
...
CreateNewSlider("Tốc Độ Chạy (Walk)", 16, 250, "WalkSpeed") -- Chức năng chỉnh Tốc độ chạy mới
CreateNewSlider("Nhảy Cao (Jump)", 50, 350, "JumpPower")   -- Chức năng chỉnh Nhảy cao mới
...
print("[GEMINIPRO]: Đã hoàn tất sửa lỗi Aimbot, thu gọn HUD và thêm tính năng chạy/nhảy!")
