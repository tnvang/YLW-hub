...
createButton("Visuals", "📷 GÓC NHÌN THỨ NHẤT (FIRST PERSON)", 5, function()
    LocalPlayer.CameraMode = Enum.CameraMode.LockFirstPerson
end)
createButton("Visuals", "🎥 GÓC NHÌN THỨ BA (THIRD PERSON)", 6, function()
...
createToggle("Visuals", "🛸 FREECAM (GÓC NHÌN TỰ DO)", 7, function(state) toggleFreecam(state) end)

-- Fake Clone
createToggle("Visuals", "👥 FAKE CLONE (BẢN SAO NHỎ)", 8, function(state) toggleFakeClone(state) end)

local CloneModeBtn = createButton("Visuals", "🤖 CLONE MODE: ĐỨNG YÊN", 9, function() end)
...
local EffBtn_Rainbow = createToggle("Visuals", "HIỆU ỨNG RAINBOW CẦU VỒNG", 10, function(state) toggleVisualEffects("Rainbow", state) end)
local EffBtn_Ice = createToggle("Visuals", "HIỆU ỨNG BĂNG GIÁ RƠI", 11, function(state) toggleVisualEffects("Ice", state) end)
local EffBtn_Smoke = createToggle("Visuals", "HIỆU ỨNG KHÓI BAY", 12, function(state) toggleVisualEffects("Smoke", state) end)
