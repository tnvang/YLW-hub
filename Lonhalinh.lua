...
----------------------------------------------------
-- CÁC HÀM LOGIC CỐT LÕI (ĐƯA LÊN TRƯỚC ĐỂ TRÁNH LỖI NIL)
----------------------------------------------------
...
local function isVisible(targetChar, targetPart)
    ...
    local ray = workspace:Raycast(Camera.CFrame.Position, targetPart.Position - Camera.CFrame.Position, p)
    return ray == nil
end

-- SỬA LỖI QUÉT TRÚNG QUÁI NPC CHÍNH XÁC KHÔNG BỊ SÓT
local function getClosestAimTarget()
    local closest, shortestDist = nil, math.huge
    local center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
    
    local function checkTarget(char, part)
        if char and part and char:FindFirstChildOfClass("Humanoid") and char:FindFirstChildOfClass("Humanoid").Health > 0 then
            local screenPos, onScreen = Camera:WorldToViewportPoint(part.Position)
            if onScreen and isVisible(char, part) then
                local dist = (Vector2.new(screenPos.X, screenPos.Y) - center).Magnitude
                if dist <= State.Aim_Radius and dist < shortestDist then
                    closest = {Character = char, Part = part}
                    shortestDist = dist
                end
            end
        end
    ...
    -- Quét Người chơi (Players)
    if State.Aim_Mode == "All" or State.Aim_Mode == "Players" then
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Head") then
                checkTarget(p.Character, p.Character.Head)
            end
        end
    end

    -- Quét Quái vật (NPCs) dựa trên Humanoid có trong Workspace công khai
    if State.Aim_Mode == "All" or State.Aim_Mode == "NPCs" then
        for _, v in ipairs(workspace:GetDescendants()) do
            if v:IsA("Model") and v:FindFirstChild("Head") and v:FindFirstChildOfClass("Humanoid") then
                if not Players:GetPlayerFromCharacter(v) and v ~= LocalPlayer.Character then
                    checkTarget(v, v.Head)
                end
            end
        end
    end
...
local function toggleAimbot(enable)
    ...
    if enable then
        ...
        aimConnection = RunService.RenderStepped:Connect(function()
            ...
            if currentAimTarget and currentAimTarget.Character and currentAimTarget.Character.Parent and currentAimTarget.Character:FindFirstChildOfClass("Humanoid") and currentAimTarget.Character:FindFirstChildOfClass("Humanoid").Health > 0 and isVisible(currentAimTarget.Character, currentAimTarget.Part) then
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not State.Hitbox_Originals[rootPart] then 
                        State.Hitbox_Originals[rootPart] = { Size = rootPart.Size, Transparency = rootPart.Transparency, CanCollide = rootPart.CanCollide } 
                    end
...
----------------------------------------------------
-- KHỞI TẠO GUI CHÍNH (ĐÃ BO TRÒN GÓC & VIỀN RAINBOW)
----------------------------------------------------
...
-- TẠO ĐƯỜNG VIỀN KHUNG (Sẽ chạy đổi màu Rainbow)
local MainStroke = Instance.new("UIStroke") 
MainStroke.Thickness = 2
MainStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
MainStroke.Parent = MainFrame

-- Vòng lặp đổi màu Rainbow mượt mà cho đường viền
task.spawn(function()
    local hue = 0
    while task.wait(0.01) do
        ...
        if MainStroke then MainStroke.Color = rainbowColor end
    end
end)
