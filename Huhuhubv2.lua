...
local VisualTab = Window:CreateTab(LangTable[CurrentLang].TabVisuals, 0)
local PlayerTab = Window:CreateTab(LangTable[CurrentLang].TabPlayer, 0)
local PvpTab = Window:CreateTab(LangTable[CurrentLang].TabCombat, 0)
local TeleTab = Window:CreateTab(LangTable[CurrentLang].TabTeleport, 0)
local SystemTab = Window:CreateTab(LangTable[CurrentLang].TabSystem, 0)
local SettingTab = Window:CreateTab(LangTable[CurrentLang].TabSettings, 0)
...
local LangDropdown = SettingTab:CreateDropdown({
   ...
   Callback = function(Option)
       ...
       InfJumpToggle:SetTitle(LangTable[CurrentLang].InfJump)
       WalkspeedSlider:SetTitle(LangTable[CurrentLang].Walkspeed)
       JumpPowerSlider:SetTitle(LangTable[CurrentLang].JumpPower)
       NoclipToggle:SetTitle(LangTable[CurrentLang].Noclip)
       HitboxSlider:SetTitle(LangTable[CurrentLang].Hitbox)
       AimModeDropdown:SetTitle(LangTable[CurrentLang].AimMode)
       AimPosDropdown:SetTitle(LangTable[CurrentLang].AimPos)
       PlayerDropdown:SetTitle(LangTable[CurrentLang].SelectPlr)
       RefreshPlrButton:SetTitle(LangTable[CurrentLang].RefreshPlr)
       TelePlrButton:SetTitle(LangTable[CurrentLang].TelePlr)
       FixLagButton:SetTitle(LangTable[CurrentLang].FixLag)
       LowGraphButton:SetTitle(LangTable[CurrentLang].LowGraph)
       AntiAfkToggle:SetTitle(LangTable[CurrentLang].AntiAfk)
       LangDropdown:SetTitle(LangTable[CurrentLang].LangSelect)
       UserParagraph:SetTitle(LangTable[CurrentLang].UserTitle)
       UserParagraph:SetContent(string.format(LangTable[CurrentLang].UserContent, LocalPlayerName))
       AuthorSection:SetTitle(LangTable[CurrentLang].AuthorSec)
       AuthorParagraph:SetTitle(LangTable[CurrentLang].AuthorTitle)
       AuthorParagraph:SetContent(LangTable[CurrentLang].AuthorContent)
       Rayfield:Notify({Title = "System", Content = LangTable[CurrentLang].NotifyLang})
   end,
})

Rayfield:Notify({
   Title = LangTable[CurrentLang].NotifyTitle,
   Content = LangTable[CurrentLang].NotifyContent,
   Duration = 5,
})
