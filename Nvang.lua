...
LoadText.Text = "Nvang: Initializing..."
...
task.spawn(function()
    ...
    for percent = 1, 100 do
        if percent % 25 == 0 then
            ...
            LoadText.Text = "Nvang: " .. statusTexts[textIdx] .. " (" .. percent .. "%)"
        else
            LoadText.Text = "Nvang: Scanning... (" .. percent .. "%)"
        end
        task.wait(0.12)
    end
    LoadFrame:Destroy()
...
local function CreateBox(obj, color)
    if obj:FindFirstChild("NvangBox") then return end
        ...
        if obj:IsA("Model") or obj:IsA("BasePart") then
            ...
            if root and (root.Position - (obj:IsA("Model") and obj:GetPivot().Position or obj.Position)).Magnitude <= 700 then
                ...
                elseif E_Vat and CheckItemESP(name) then
                    ...
                    if obj:FindFirstChild("NvangBox") then obj.NvangBox:Destroy() end
                end
            else
                if obj:FindFirstChild("NvangBox") then obj.NvangBox:Destroy() end
            end
...
ResetBtn.MouseButton1Click:Connect(function()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do if obj:FindFirstChild("NvangBox") then obj.NvangBox:Destroy() end end
end)
