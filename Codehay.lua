...
if _G.MobileHubExecutedFinalV3 then
    print("[Mobile Hub] Hub đã được kích hoạt trước đó!")
    return
else
    _G.MobileHubExecutedFinalV3 = true
end
...
ScreenGui.Name = "UniversalMobileHub_FinalV3"
...
Title.Text = "PREMIUM MOBILE HUB V3"
...
local Hitbox_Btn = createToggle("Hitbox_Btn", "HITBOX EXPANDER: OFF", 4)

local SliderContainer = Instance.new("Frame")
SliderContainer.Size = UDim2.new(0, 240, 0, 20)
SliderContainer.BackgroundTransparency = 1
SliderContainer.LayoutOrder = 5
SliderContainer.Parent = ScrollContainer
...
SliderButton.Position = UDim2.new(0, 0, -0.5, -1)
SliderButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
SliderButton.Text = ""
SliderButton.Parent = SliderFrame
local Scrn = Instance.new("UICorner") Scrn.CornerRadius = UDim.new(1, 0) Scrn.Parent = SliderButton

local SizeLabel = Instance.new("TextLabel")
SizeLabel.Size = UDim2.new(0, 70, 1, 0)
SizeLabel.Position = UDim2.new(0, 170, 0, 0)
...
TransContainer.Size = UDim2.new(0, 240, 0, 25)
...
TransBox.Size = UDim2.new(0, 45, 1, 0)
...
local Tcrn = Instance.new("UICorner") Tcrn.CornerRadius = UDim.new(0, 5) Tcrn.Parent = TransBox

local TransLabel = Instance.new("TextLabel")
TransLabel.Size = UDim2.new(0, 180, 1, 0)
TransLabel.Position = UDim2.new(0, 55, 0, 0)
...
local ESPName_Btn = createToggle("ESPName_Btn", "ESP NAME: OFF", 7)
local ESPBox_Btn = createToggle("ESPBox_Btn", "ESP BOX: OFF", 8)
local Speed_Btn = createToggle("Speed_Btn", "SPEED (100): OFF", 9)
local Jump_Btn = createToggle("Jump_Btn", "JUMP (120): OFF", 10)
local InfJump_Btn = createToggle("InfJump_Btn", "INFINITE JUMP: OFF", 11)
local Fly_Btn = createToggle("Fly_Btn", "FLY MOBILE: OFF", 12)
local Noclip_Btn = createToggle("Noclip_Btn", "NOCLIP (XUYÊN TƯỜNG): OFF", 13)
local PortalMaster_Btn = createToggle("PortalMaster_Btn", "HỆ THỐNG PORTAL: OFF", 15)
...
TeleportContainer.Size = UDim2.new(0, 240, 0, 35)
TeleportContainer.BackgroundTransparency = 1
TeleportContainer.LayoutOrder = 14
...
local Spbc = Instance.new("UICorner") Spbc.CornerRadius = UDim.new(0, 6) Spbc.Parent = SelectPlayerBtn

local DropdownFrame = Instance.new("ScrollingFrame")
DropdownFrame.Size = UDim2.new(0, 240, 0, 120)
...
local Dpcf = Instance.new("UICorner") Dpcf.CornerRadius = UDim.new(0, 6) Dpcf.Parent = DropdownFrame
...
local function updateDropdownList()
    ...
    DropdownFrame.CanvasSize = UDim2.new(0, 0, 0, totalPlayers * 30)
end
...
PortalContainer.Size = UDim2.new(0, 240, 0, 85)
PortalContainer.BackgroundTransparency = 1
PortalContainer.LayoutOrder = 16
...
for i = 1, MAX_PORTALS do
	...
	btn.Position = UDim2.new(0, (i-1)*36 + 6, 0, 5)
	btn.Text = tostring(i)
	btn.TextSize = 14
	btn.Font = Enum.Font.SourceSansBold
	btn.TextColor3 = Color3.fromRGB(255, 255, 255)
	...
	delBtn.Size = UDim2.new(0, 32, 0, 22)
	delBtn.Position = UDim2.new(0, (i-1)*36 + 6, 0, 42)
	delBtn.Text = "X"
	delBtn.TextSize = 12
...
clearBtn.Size = UDim2.new(0, 50, 0, 59)
clearBtn.Position = UDim2.new(0, 185, 0, 5)
clearBtn.Text = "Clear\nAll"
...
local function toggleFly(enable)
    ...
    if enable then
        Fly_Btn.Text = "FLY MOBILE: ON"; Fly_Btn.BackgroundColor3 = Color3.fromRGB(50, 180, 50)
        task.spawn(function()
            local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
            local root = char:WaitForChild("HumanoidRootPart")
            FlyVelocity = Instance.new("BodyVelocity") FlyVelocity.MaxForce = Vector3.new(1e5, 1e5, 1e5) FlyVelocity.Velocity = Vector3.new(0, 0, 0) FlyVelocity.Parent = root
            while State.Fly_Enabled and char and root and FlyVelocity.Parent do
                ...
                if LocalPlayer.Character:FindFirstChildOfClass("Humanoid").MoveDirection.Magnitude > 0 then
                    dir = LocalPlayer.Character:FindFirstChildOfClass("Humanoid").MoveDirection * State.Fly_Speed
                end
        ...
        Fly_Btn.Text = "FLY MOBILE: OFF"; Fly_Btn.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
...
local function optimizeObject(v)
    ...
    if v:IsA("BasePart") then v.Material = Enum.Material.SmoothPlastic; v.CastShadow = false; v.Reflectance = 0
    elseif v:IsA("ParticleEmitter") or v:IsA("Smoke") or v:IsA("Fire") or v:IsA("Sparkles") or v:IsA("Trail") or v:IsA("Beam") then v:Destroy() end
end
...
local function toggleAimbot(enable)
    ...
    if enable then
        Aim_Btn.Text = "AIMBOT: ON"; Aim_Btn.BackgroundColor3 = Color3.fromRGB(50, 180, 50); FOVCircle.Visible = true
...
SliderButton.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then isSliding = true end end)
UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then isSliding = false end end)
UserInputService.InputChanged:Connect(function(i)
    if isSliding and (i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseMovement) then
        local pct = math.clamp((i.Position.X - SliderFrame.AbsolutePosition.X) / SliderFrame.AbsoluteSize.X, 0, 1)
        SliderButton.Position = UDim2.new(pct, -7, -0.5, -1)
        State.Hitbox_Size = math.clamp(math.round(pct * 500), 5, 500)
        SizeLabel.Text = "Size: " .. tostring(State.Hitbox_Size)
    end
end)
...
Speed_Btn.MouseButton1Click:Connect(function()
    ...
    Speed_Btn.Text = State.WalkSpeed_Enabled and "SPEED (100): ON" or "SPEED (100): OFF"
...
Jump_Btn.MouseButton1Click:Connect(function()
    ...
    Jump_Btn.Text = State.JumpPower_Enabled and "JUMP (120): ON" or "JUMP (120): OFF"
