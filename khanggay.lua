...
LoadText.Text = "khanggay: Initializing..."
...
task.spawn(function()
    ...
    for percent = 1, 100 do
        if percent % 25 == 0 then
            local textIdx = math.clamp(percent / 25, 1, #statusTexts)
            LoadText.Text = "khanggay: " .. statusTexts[textIdx] .. " (" .. percent .. "%)"
        else
            LoadText.Text = "khanggay: Scanning... (" .. percent .. "%)"
        end
        task.wait(0.12)
    end
    LoadFrame:Destroy()
...
local function CreateBox(obj, color)
    if obj:FindFirstChild("khanggayBox") then return end
    local box = Instance.new("BoxHandleAdornment")
    box.Name = "khanggayBox"
        ...
        if obj:IsA("Model") or obj:IsA("BasePart") then
            ...
            if root and (root.Position - (obj:IsA("Model") and obj:GetPivot().Position or obj.Position)).Magnitude <= 700 then
                ...
                elseif E_Vat and CheckItemESP(name) then
                    ...
                    if obj:FindFirstChild("khanggayBox") then obj.khanggayBox:Destroy() end
                end
            else
                if obj:FindFirstChild("khanggayBox") then obj.khanggayBox:Destroy() end
            end
...
local function SetNoclipState(state)
    ...
    if Noclip then
        NoclipConnection = RunService.Stepped:Connect(function()
            if Player.Character then for _, p in pairs(Player.Character:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide = false end end end end
        end)
    else
...
-- VÒNG LẶP XỬ LÝ AIMBOT ĐÃ ĐƯỢC CẬP NHẬT THEO ĐIỀU KIỆN CHI TIẾT CỦA BẠN
RunService.RenderStepped:Connect(function()
    if AimBot then
        local closest = nil
        local dist = 700
        ...
        for _, v in pairs(Workspace:GetDescendants()) do
            if v:IsA("Model") and v:FindFirstChild("Humanoid") then
                ...
                -- Điều kiện: Quái còn sống (Health > 0), không phải Player khác, không phải chính mình
                if hum.Health > 0 and not isPlayer and v ~= Player.Character then
                    -- Khóa thẳng đầu nha (Tìm bộ phận Head)
        ...
        if closest then
            local targetCFrame = CFrame.new(Camera.CFrame.p, closest.Position)
...
ResetBtn.MouseButton1Click:Connect(function()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do if obj:FindFirstChild("khanggayBox") then obj.khanggayBox:Destroy() end end
end)
